...
//    }
    ...
    func locationManager(_ manager: CLLocationManager, didChangeAuthorization status: CLAuthorizationStatus) {
       ...
       let accessGranted = (status == CLAuthorizationStatus.authorizedAlways ||
            status == CLAuthorizationStatus.authorizedWhenInUse)
        
        print("<LocationManager> Access Granted = \(accessGranted)")
        NotificationCenter.default.post(name: .LocationAccessStatusChanged, object: accessGranted)
    }
...
extension Notification.Name {
    static let LocationAccessStatusChanged = Notification.Name("LocationAccessStatusChanged")
}
